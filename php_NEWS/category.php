...
$printPostQuery = "SELECT p.post_id, p.title, p.post_date, p.post_img, p.description, p.category, c.category_name, c.category_id, u.username, u.user_id FROM post p
                  INNER JOIN category c
                  ON p.category = c.category_id
                  INNER JOIN user u
                  ON p.author = u.user_id
                  WHERE p.category = {$categoryIdUrl}
      ...
      <div class="row">
         <div class="col-md-8">
            ...
            <div class="post-container">
               ...
               ?>
                     <h2 class="page-heading"><?php echo $postRow['category_name'] ?></h2>
                           ...
                           <div class="col-md-8">
                              <div class="inner-content clearfix">
                                 ...
                                 <div class="post-information">
                                    ...
                                    <span>
                                       <i class="fa fa-calendar" aria-hidden="true"></i><?php echo $postRow['post_date'] ?></span>
                                 </div>
                  ...
                  $totalRecordsQuery = "SELECT * FROM post WHERE category = {$categoryIdUrl}";
                  ...
                  <?php
                     if ($currentPageNumber > 1) {
                        echo '<li><a href="category.php?pageNo=' . ($currentPageNumber - 1) . '&catIdUrl=' . $categoryIdUrl . '">Prev</a></li>';
                     }
                     for ($p = 1; $p <= $totalPages; $p++) {
                        ...
                        echo "<li class='{$cssClass}'><a href='category.php?pageNo={$p}&catIdUrl={$categoryIdUrl}'> $p </a></li>";
                     }
                     if ($currentPageNumber < $totalPages) {
                        echo '<li><a href="category.php?pageNo=' . ($currentPageNumber + 1) . '&catIdUrl=' . $categoryIdUrl . '">Next</a></li>';
                     }
                  }
               } else {
                  echo "<div class='alert alert-success mt-5 h3 text-center'>No Post About This Category For The Moment.</div>";
               }
                  ?>
            </div>
            <!-- /post-container End -->
